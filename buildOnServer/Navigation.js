...
import { faGaugeHigh, faFileInvoice, faUserCircle, faMapMarkerAlt, faBarcode, faUser } from '@fortawesome/free-solid-svg-icons'
...
function Navigation() {
  ...
  useEffect(() => {
    ...
    if (pathname === '/vsell/Dashboard') {
      setActiveComponent('Dashboard');
    } else if (pathname === '/vsell/KhachHang') {
      setActiveComponent('KhachHang');
    } else if (pathname === '/vsell/KhuVuc') {
      setActiveComponent('KhuVuc');
    } else if (pathname === '/vsell/MatHang') {
      setActiveComponent('MatHang');
    } else if (pathname === '/vsell/NhanVien') {
      ...
      <aside style={{ backgroundColor: '#FAFAFA' }} class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 ps ps--active-y" id="sidenav-main">
        <div class="sidenav-header">
          <i class="fas fa-times p-3 cursor-pointer text-secondary opacity-5 position-absolute end-0 top-0 d-none d-xl-none" aria-hidden="true" id="iconSidenav"></i>
            ...
            <img src={logo} class="navbar-brand-img h-200" alt="main_logo" />
        ...
        <hr class="horizontal dark mt-0" />
          ...
          <ul class="navbar-nav">
            <li class="nav-item">
              <Link id="Dashboard" className={`nav-link${activeComponent === 'Dashboard' ? ' active' : ''}`} to="/vsell/Dashboard">
                <FontAwesomeIcon icon={faGaugeHigh} />
                <span class="nav-link-text ms-1"> Bảng Điều Khiển</span>
              </Link>
            </li>
            <li class="nav-item">
              <Link id="HoaDon" className={`nav-link${activeComponent === 'HoaDon' ? ' active' : ''}`} to="/vsell/">
                <FontAwesomeIcon icon={faFileInvoice} />
                <span class="nav-link-text ms-1">  Hoá Đơn</span>
              </Link>
            </li>
            <li class="nav-item">
              <Link id="KhachHang" className={`nav-link${activeComponent === 'KhachHang' ? ' active' : ''}`} to="/vsell/KhachHang">
                <FontAwesomeIcon icon={faUserCircle} />
                <span class="nav-link-text ms-1"> Khách Hàng</span>
              </Link>
            </li>
            <li class="nav-item">
              <Link id="KhuVuc" className={`nav-link${activeComponent === 'KhuVuc' ? ' active' : ''}`} to="/vsell/KhuVuc">
                <FontAwesomeIcon icon={faMapMarkerAlt} />
                <span class="nav-link-text ms-1">  Khu Vực</span>
              </Link>
            </li>
            <li class="nav-item">
              <Link id="MatHang" className={`nav-link${activeComponent === 'MatHang' ? ' active' : ''}`} to="/vsell/MatHang">
                <FontAwesomeIcon icon={faBarcode} />
                <span class="nav-link-text ms-1"> Mặt Hàng</span>
              </Link>
            </li>
            <li class="nav-item">
              <Link id="NhanVien" className={`nav-link${activeComponent === 'NhanVien' ? ' active' : ''}`} to="/vsell/NhanVien">
