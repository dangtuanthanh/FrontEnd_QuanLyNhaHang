...
// dùng để reset khi bấm nút quay lại
    ...
    function handleSelectAllChange(event) {
        ...
        if (isChecked) {
            ...
            const allIds = props.duLieuHienThi.map((item) => item.IDDonViTinh.toString());
        ...
        <table class="table align-items-center mb-0">
            <thead>
                <tr >
                    ...
                    <th style={{ textAlign: 'center' }} onClick={() => handleClickSort('IDDonViTinh')} class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-10">ID Đơn Vị Tính </th>
                    <th style={{ padding: 8 }} onClick={() => handleClickSort('TenDonViTinh')} class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-10">Tên Đơn Vị Tính</th>
            ...
            <tbody>
                {
                    props.duLieuHienThi.map((dulieu, index) =>
                        ...
                        id='trdata' 
                        key={dulieu.IDDonViTinh} 
                        onClick={() => {
                            props.setIsInsert(false)
                            props.setIDAction(dulieu.IDDonViTinh)
                            props.setPopupInsertUpdate(true)
                        }} >
                            <td >
                                <input
                                    type="checkbox"
                                    value={dulieu.IDDonViTinh}
                                    className='checkboxCon'
                                    checked={props.selectedIds.includes(dulieu.IDDonViTinh.toString())}
                            ...
                            <td>
                                ...
                                <a onClick={(e) => {
                                    e.stopPropagation(); props.openPopupAlert(
                                        `Bạn có chắc chắn muốn xoá ${dulieu.TenDonViTinh}? Một số ràng buộc liên quan đến dữ liệu này sẽ bị mất dẫn đến việc hiển thị dữ liệu không chính xác!`,
                                        () => { props.deleteData(dulieu.IDDonViTinh) }
                                    )
