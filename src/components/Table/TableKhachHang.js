...
// dùng để reset khi bấm nút quay lại
    ...
    function handleSelectAllChange(event) {
        ...
        if (isChecked) {
            ...
            const allIds = props.duLieuHienThi.map((item) => item.IDKhachHang.toString());
        ...
        <table class="table align-items-center mb-0">
            <thead>
                <tr >
                    ...
                    <th style={{ textAlign: 'center' }} onClick={() => handleClickSort('IDKhachHang')} class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-10">ID Khu Vực </th>
                    ...
                    <th style={{ textAlign: 'center'}} onClick={() => handleClickSort('SoDienThoai')} class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-10">Số Điện Thoại </th>
            ...
            <tbody>
                {
                    props.duLieuHienThi.map((dulieu, index) =>
                        ...
                        onClick={() => {
                            ...
                            props.setIDAction(dulieu.IDKhachHang)
                            props.setPopupInsertUpdate(true)
                        }} >
                            <td >
                                <input
                                    type="checkbox"
                                    value={dulieu.IDKhachHang}
                                    className='checkboxCon'
                                    checked={props.selectedIds.includes(dulieu.IDKhachHang.toString())}
                            ...
                            <td>
                                ...
                                <a onClick={(e) => {
                                    e.stopPropagation(); props.openPopupAlert(
                                        `Bạn có chắc chắn muốn xoá ${dulieu.TenKhachHang}`,
                                        () => { props.deleteData(dulieu.IDKhachHang) }
                                    )
