...
// dùng để reset khi bấm nút quay lại
    ...
    function handleSelectAllChange(event) {
        ...
        if (isChecked) {
            ...
            const allIds = props.duLieuHienThi.map((item) => item.IDKhuVuc.toString());
        ...
        <table class="table align-items-center mb-0">
            <thead>
                <tr >
                    ...
                    <th style={{ textAlign: 'center' }} onClick={() => handleClickSort('IDKhuVuc')} class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-10">ID Khu Vực </th>
                    <th style={{ padding: 8 }} onClick={() => handleClickSort('TenKhuVuc')} class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-10">Tên Khu Vực </th>
            ...
            <tbody>
                {
                    props.duLieuHienThi.map((dulieu, index) =>
                        ...
                        onClick={() => {
                            ...
                            props.setIDAction(dulieu.IDKhuVuc)
                            props.setPopupInsertUpdate(true)
                        }} >
                            <td >
                                <input
                                    type="checkbox"
                                    value={dulieu.IDKhuVuc}
                                    className='checkboxCon'
                                    checked={props.selectedIds.includes(dulieu.IDKhuVuc.toString())}
                            ...
                            {/* <td >{index + 1}</td> */}
                            <td >{dulieu.IDKhuVuc}</td>
                            <td style={{ textAlign: 'left' }} >{dulieu.TenKhuVuc}</td>
                            <td>
                                <a onClick={(e) => {
                                    e.stopPropagation();
                                    props.setIsInsert(false)
                                ...
                                <a onClick={(e) => {
                                    e.stopPropagation(); props.openPopupAlert(
                                        `Bạn có chắc chắn muốn xoá ${dulieu.TenKhuVuc}? Một số ràng buộc liên quan đến dữ liệu này sẽ bị mất dẫn đến việc hiển thị dữ liệu không chính xác!`,
                                        () => { props.deleteData(dulieu.IDKhuVuc) }
                                    )
