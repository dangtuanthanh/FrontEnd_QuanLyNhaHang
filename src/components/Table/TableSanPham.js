...
const TableSanPham = (props) => {
    ...
    function handleSelectAllChange(event) {
        ...
        if (isChecked) {
            ...
            const allIds = props.duLieuHienThi.map((item) => item.IDSanPham.toString());
                    ...
                    props.duLieuHienThi.map((dulieu, index) =>
                        <tr
                            ...
                            <td >
                                <input
                                    type="checkbox"
                                    value={dulieu.IDSanPham}
                                    className='checkboxCon'
                                    checked={props.selectedIds.includes(dulieu.IDSanPham.toString())}
                            ...
                            <td>
                                ...
                                <a onClick={(e) => {
                                    e.stopPropagation(); props.openPopupAlert(
                                        `Bạn có chắc chắn muốn xoá ${dulieu.TenSanPham}? Một số ràng buộc liên quan đến dữ liệu này sẽ bị mất dẫn đến việc hiển thị dữ liệu không chính xác!`,
                                        () => { props.deleteData(dulieu.IDSanPham) }
                                    )
