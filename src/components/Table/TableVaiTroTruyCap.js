...
// dùng để reset khi bấm nút quay lại
    ...
    function handleSelectAllChange(event) {
        ...
        if (isChecked) {
            ...
            const allIds = props.duLieuHienThi.map((item) => item.IDVaiTro.toString());
        ...
        <table class="table align-items-center mb-0">
            <thead>
                <tr >
                    ...
                    <th style={{ textAlign: 'center' }} onClick={() => handleClickSort('IDVaiTro')} class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-10">ID Vai Trò </th>
            ...
            <tbody>
                {
                    props.duLieuHienThi.map((dulieu, index) =>
                        ...
                        onClick={() => {
                            ...
                            props.setIDAction(dulieu.IDVaiTro)
                            props.setPopupInsertUpdate(true)
                        }} >
                            <td >
                                <input
                                    type="checkbox"
                                    value={dulieu.IDVaiTro}
                                    className='checkboxCon'
                                    checked={props.selectedIds.includes(dulieu.IDVaiTro.toString())}
                            ...
                            <td>
                                ...
                                <a onClick={(e) => {
                                    e.stopPropagation(); props.openPopupAlert(
                                        `Bạn có chắc chắn muốn xoá ${dulieu.TenVaiTro}? Một số ràng buộc liên quan đến dữ liệu này sẽ bị mất dẫn đến việc hiển thị dữ liệu không chính xác!`,
                                        () => { props.deleteData(dulieu.IDVaiTro) }
                                    )
