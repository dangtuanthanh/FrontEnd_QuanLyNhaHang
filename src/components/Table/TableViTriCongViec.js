...
const TableViTriCongViec = (props) => {
...
// dùng để reset khi bấm nút quay lại
    ...
    function handleSelectAllChange(event) {
        ...
        if (isChecked) {
            ...
            const allIds = props.duLieuHienThi.map((item) => item.IDViTriCongViec.toString());
        ...
        <table class="table align-items-center mb-0">
            <thead>
                <tr >
                    ...
                    <th style={{ textAlign: 'center' }} onClick={() => handleClickSort('IDViTriCongViec')} class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-10">ID Vị Trí Công Việc </th>
                    <th style={{ padding: 8 }} onClick={() => handleClickSort('TenViTriCongViec')} class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-10">Tên Vị Trí Công Việc </th>
            ...
            <tbody>
                {
                    props.duLieuHienThi.map((dulieu, index) =>
                        ...
                        id='trdata' 
                        key={dulieu.IDViTriCongViec} 
                        onClick={() => {
                            props.setIsInsert(false)
                            props.setIDAction(dulieu.IDViTriCongViec)
                            props.setPopupInsertUpdate(true)
                        }} >
                            <td >
                                <input
                                    type="checkbox"
                                    value={dulieu.IDViTriCongViec}
                                    className='checkboxCon'
                                    checked={props.selectedIds.includes(dulieu.IDViTriCongViec.toString())}
...
export default TableViTriCongViec;