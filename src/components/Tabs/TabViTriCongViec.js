...
import { urlGetJobPosition, urlDeleteJobPosition } from "../url";
import Pagination from "../Pagination";
import ItemsPerPage from "../ItemsPerPage";
import TableViTriCongViec from "../Table/TableViTriCongViec";
import Insert_updateJobPosition from "../Popup/Insert_updateJobPosition";
function TabViTriCongViec() {
    //xử lý redux
    ...
    const [dataUser, setdataUser] = useState({//dữ liệu người dùng
        sortBy: 'IDViTriCongViec',
        sortOrder: 'asc',
        searchBy: 'TenViTriCongViec',
        search: '',
        searchExact: 'false'
    });//
    ...
    const TaiDuLieu = () => {
        dispatch({ type: 'SET_LOADING', payload: true })
        fetch(`${urlGetJobPosition}?page=${dataUser.page}&limit=${dataUser.limit}&sortBy=${dataUser.sortBy}&sortOrder=${dataUser.sortOrder}&search=${dataUser.search}&searchBy=${dataUser.searchBy}&searchExact=${dataUser.searchExact}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'ss': getCookie('ss'),
        ...
        <div>
            <div class="card" style={{ minHeight: '92vh', position: 'relative' }}>
                <div class="card-header pb-0">
                    <h2 onClick={handleToggleButtonFunction}> Quản Lý Vị Trí Công Việc  <button type="button" className="btn btn-link btn-sm mb-0 " style={{ width: '100px', float: 'right' }}><FontAwesomeIcon icon={showButtonFunction ? faArrowUp : faArrowDown} /></button></h2>
                    ...
                    <div>
                        ...
                        <div style={{ 'display': "inline-block", float: 'right' }}>
                            ...
                            <select class="form-select-sm" value={dataUser.searchBy} onChange={handleSearchBy}>
                                <option value="IDViTriCongViec">Tìm theo ID Vị Trí Công Việc</option>
            ...
            </div>
            {
                popupInsertUpdate && <div className="popup">
                    <Insert_updateJobPosition
                        isInsert={isInsert}
                        setPopupInsertUpdate={setPopupInsertUpdate}
                        tieuDe='Thông Tin Vị Trí Công Việc'
                        dataUser={dataUser}
                        setdataUser={setdataUser}
                        addNotification={addNotification}
                        openPopupAlert={openPopupAlert}
...
export default TabViTriCongViec