...
function HoaDon() {
    ...
    useEffect(() => {
        getData(sessionStorage.getItem("pageNhanVienTable"));
    }, []);
    ...
    const getData = async (page) => {
        ...
        var Last = page * sessionStorage.getItem("rowNhanVienTable");
        var First = Last - sessionStorage.getItem("rowNhanVienTable");
        setNhanvien(dulieu.slice(First, Last));
        // const viTriBatDauDuLieu = trangHienTai * row;
        // duLieuTrangHienTai = data.slice(viTriBatDauDuLieu, viTriBatDauDuLieu + row);
    }
    // Hết tải dữ liệu
    {/*Thêm dữ liệu  */ }
        ...
        <div >
            ...
            <div class="card mb-4">
                ...
                <div class="card-header pb-0">
                    ...
                    <div>
                        <button style={{ display: "inline-block" }} onClick={() => getData(sessionStorage.getItem("pageNhanVienTable"))} className="btn btn-primary"><FontAwesomeIcon icon={faRotate} />ㅤLàm Mới Dữ Liệu </button>ㅤ
                        ...
                        <div style={{ display: "inline-block", float: 'right' }}>
                            <select class="form-select-sm" value={sessionStorage.getItem("rowNhanVienTable")} id="hienthidulieu" onChange={() => { dataUser($("#hienthidulieu").val()); getData(1) }}>
                            ...
                            <tbody>
                                <img className='loading' style={{ position: "fixed", width: "20%", left: "50%", top: "40%" }} src={loading1}></img>
                                {
                                    nhanvien.map((nhanvien, index) =>
                                        <tr id='trdata' key={nhanvien.manv}>
                                            <td >{index + 1}</td>
                                            <td >{nhanvien.SoHD}</td>
